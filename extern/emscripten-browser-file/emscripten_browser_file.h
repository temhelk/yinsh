...
EM_JS_INLINE(void, upload, (char const *accept_types, upload_handler callback, void *callback_data), {
  ...
  globalThis["open_file"] = function(e) {
    ...
    file_reader.onload = (event) => {
      ...
      ccall('upload_file_return', 'number', ['string', 'string', 'number', 'number', 'number', 'number'], [event.target.filename, event.target.mime_type, data_on_heap.byteOffset, uint8Arr.length, callback, callback_data]);
      _free(data_ptr);
    };
  ...
  file_selector.addEventListener('cancel', () => {
    ccall('upload_file_return', 'number', ['string', 'string', 'number', 'number', 'number', 'number'], ["", "", 0, 0, callback, callback_data]);
  });
